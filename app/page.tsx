...
import { Tv, Gamepad2, ImageIcon, Music, Activity, Video } from "lucide-react"
...
const apps = [
  ...
  { id: "video", icon: Video, color: "bg-green-500", name: "Music" },
        ...
        >
          <div className="grid grid-cols-6 gap-9">
            {apps.map((app) => (
              <motion.button
                key={app.id}
                className={`relative group flex flex-col items-center group-hover:shadow-lg`}
              ...
              >
                <motion.div
                  className={`aspect-video w-full rounded-2xl flex items-center justify-center ${app.color}`}
                  animate={{
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-6 gap-9 relative">
          {Array.from({ length: 18 }).map((_, i) => (
            <motion.button
              ...
              className={`relative group flex flex-col items-center group-hover:shadow-2xl group-focus:shadow-lg`}
